...
public class Controller {
	...
	public Controller(ModelRoot model, RootView view) {
		...
		view.setMenuDetailsBtnDeleteDishPermentlyFromListEventHandler(
				new EHMenuDetailsBtnDeleteDishPermentlyFromList());
		...
		view.setDishDetailsBtnAboutEventHandler((ActionEvent event) -> {
			model.makeInfoAlert(
					"This is the page where you can make a new dish which will be shown in the menu list page").show();
		});
	 ...
	 * if an issue is present it shows an error message, if pass validation the
	 * password is then hashed (SHA-256) and checked against the database to see if
	 * it matches it loads the home page, type load based on the the account that is
	 * logged in, if the password and user name doesn't match it again shows an
	 * error message. error message done by alert dialog box.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * class loads the menu list page and populate its list view with all the menus
	 * in the database.
	 * 
	 * @author Student
	 *
	...
	/**
	 * loads the menu details page. it sets the from menu to null so that it know
	 * its an add and not an edit, it also populates the combo box, with all the
	 * dishes the database has.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * loads the delete page. it checks the user has selected an item from the list
	 * view if no item has been selected it make the error message label visible
	 * saying the issue if an item has been selected it loads the delete
	 * confirmation page with the name of the item to be deleted in it.
	 * 
	 * 
	 * @author Student
	 *
	...
	private class EHAccountListBtnDelete implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			if (!view.getAccountListSelectedItem().equals("null")) {
				...
				if ((model.getSelectedAccountAdminStatus().equals(false)
	...
	/**
	 * loads the dish details page. sets the came from edit to false and details
	 * original id to null so it knows that it is, adding not editing.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * loads the DishDetails page. check the user has selected a dish from the list
	 * view. if no dish has been selected an error message is shown, if a dish has
	 * been shown it loads the dish details page, with that dish details populating
	 * the dish details list view.
	 * 
	 * @author Student
	 *
	 */
			...
			if (masterErrorMessage.equals("")) {
				...
				if (view.getMenuDetailsDishListSelectedItemIndex() != -1) {
					...
					model.setSelectedDish(
							model.getASpecificDish(view.getMenuDetailsMenuDishListSelectedItemValueIdOnly()));
				}
	 ...
	 * first it checks if the use has inputed in a menu name and a budget in the
	 * settings page if not it shows an alert error message if they have it loads a
	 * file chooser. if the user inputs a file that isnt the correct file type it
	 * will show an error message if the file has unrecognisable dishes, it will
	 * show a information alert box that dishes where not readable. if no issue
	 * happen it will populate the menu details page with the info in the file.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * loads the stock detail page. the input fields in the stock details page are
	...
	/**
	 * loads the budget detail page. the input fields in the budget details page are
	 * empty and ready for new input.
	 * 
	 * @author Student
	 *
	...
	/**
	 * loads the storage details page. checks the user has selected an item from the
	 * list view if no item has been selected it make the error label visible with
	 * info about the error if an item has been selected it loads the storage
	...
	private class EHStorageListBtnEdit implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			if (!view.getStorageListSelectedItem().equals("null")) {
				...
				model.selectAStorageLocation(view.getSelectedStorageId());
				view.setStorageLocationDetailsValues(model.getSelectedStorageName(), model.getSelectedStorageType(),
						model.getSelectedStorageAvailbilty());
	...
	/**
	 * loads the budget details page. checks the user has selected an item from the
	 * list view if no item has been selected it make the error label visible with
	 * info about the error if an item has been selected it loads the budget details
	 * page with the selected items info populating it.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * loads the account details page. checks the user has selected an item from the
	 * list view if no item has been selected it make the error label visible with
	 * info about the error if the selected item is an admin and is not the logged
	 * in admin an error message is shown if an item has been selected and isnt an
	 * admin account or is the logged in admin account it loads the account details
	 * page with the selected items info populating it.
	 * 
	 * @author Student
	 *
	...
	/**
	 * loads the storage location details page. it clears any info that is the
	 * storage location details page and then loads it.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * loads the account details page. it clears any info that is the account
	 * details page and then loads it.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * loads the stock storage location filter class/ page. loads the page and
	 * populates the combo box with all the storage types the database has.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * loads the stock filter class/ page. loads the page and populates the
	 * corresponding combo boxes with either all the stock types or storage
	 * locations the database has depending on which combo box it is.
	 * 
	 * @author Student
	 *
	 */
	private class EHStockBtnFilter implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			view.stockFilterLoad(model.getObservableStorgaeLocationListNameOnlyArrayList(),
					model.getStockTypeListAsString());
	...
	/**
	 * loads the settings page. loads the menu details setting pages and populate
	 * the combo box with all the budget that the database holds.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * loads the home page. loads the home page and sets the admin status, which
	 * determines the type of home page loaded, eg admin or none admin one.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * loads the login page. take the user from what ever page they are on to the
	 * login page which input has been cleared from the last user input.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * loads the output page. button found in the menu details page.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * saves the stock details in the database. it check the user input, if there is
	 * an issue with it show this issue in an alert dialog box
	 * 
	 * if there is no issue it will look to see if there is a stored stock name/id
	 * this is done as if they come from add there will be null but if from edit
	 * there will be an id/name if there isnt one it will simply insert/ make a new
	 * one in the database, if there is one it will update that stock type but only
	 * bits that very using the info provided.
	 *
	 * it then check the get stock from, if true it makes a new stock, if false it
	 * updates the stock this is set when the button eg add and edit is pressed,
	 * button in the stock list page. it then loads the stock list page.
	 *
	 * @author Student
	 *
	 */
			...
			if (masterErrorMessage.equals("")) {
				...
				if (output == null || output.isPresent() && output.get() == ButtonType.OK) {
					...
					if (model.getStockFrom()) {
						...
						model.createStock(-1, view.getStorageLocation(),
								Double.parseDouble(view.getStockDetailsQuantity()), view.getStockDetailsQuanitType(),
								model.formatDate(view.getStockDetailsDateText()), view.getStockDetailsStockName(),
								10.5);
						model.addCurrentStock();

					} else {

						model.createStock(model.getSelectedStockId(), view.getStorageLocation(),
								Double.parseDouble(view.getStockDetailsQuantity()), view.getStockDetailsQuanitType(),
								model.formatDate(view.getStockDetailsDateText()), view.getStockDetailsStockName(),
								10.5);
	...
	/**
	 * loads the stock list page. the stock list page is loaded and the list view is
	 * populated with all the stock that the database has in a string format.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * loads the page where the user came from and populates the corresponding list
	 * view with what the database holds.
	 * 
	 * @author Student
	 *
	 */
		...
		public void handle(ActionEvent event) {
			if (model.getDeleteFrom().equals("StockList")) {
				...
				loadStorgaeLocationListPage();
					...
					|| model.getDeleteFrom().equals("RemoveFromShoppingList")
					|| model.getDeleteFrom().equals("RemoveFromMenuList")) {
				view.MenuDetailsLoad();
	 ...
	 * check the user input if there is an issue the label is made visible and show
	 * the error if there is no issue the list view is populates with all the stock
	 * the database has that is like the user input. like in mysql is % user input
	 * %.
	 * 
	 * @author Student
	 *
	...
	/**
	 * 
	 * makes the list view show just the accounts that are like the find value
	 * input. check the user input if there is an issue the label is made visible
	 * and show the error if there is no issue the list view is populates with all
	 * the accounts in the database has that are like the user input. like in mysql
	 * is % user input %.
	 * 
	 * @author Student
	 *
	...
	/**
	 * 
	 * makes the list view show just the storage locations that are like the find
	 * value input. check the user input if there is an issue the label is made
	 * visible and show the error if there is no issue the list view is populates
	 * with all the storage locations that the database has that is like the user
	 * input. like in mysql is % user input %.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * It displays a filtered list of the stock items that are in the database based
	 * of the inputed filters.
	 * 
	 * it will first validate the input, no input is accepted. if there is an issue
	 * will display the issue in an alert dialog box. if there is no issue, it will
	 * display in the stock list page a list of all the menu that passed all the
	 * filters. the inputs are from the stock filter page.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * It displays a filtered list of the budgets that are in the database based of
	 * the inputed filters.
	 * 
	 * it will first validate the input, no input is accepted. if there is an issue
	 * will display the issue in an alert dialog box. if there is no issue, it will
	 * display in the budget list page a list of all the menu that passed all the
	 * filters. the inputs are from the budget filter page.
	 * 
	 * @author Student
	 *
	 */
	private class EHBudgetFilterBtnApply implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			String masterErrorMessage = "";
			String issueFrom = "";
			String budgetLessErrorMessage = model
			...
			if (masterErrorMessage.equals("")) {
				...
				if (!view.getBudgetFilterNoBudgetLessThan().equals("")) {
					whereClause = whereClause + "tbl_budget.amount >= \"" + view.getBudgetFilterNoBudgetLessThan()
							+ "\" And ";
				}
				if (!view.getBudgetFilterNoBudgetMoreThan().equals("")) {
					whereClause = whereClause + "tbl_budget.amount <= \"" + view.getBudgetFilterNoBudgetMoreThan()
							+ "\" And ";
				}

				if (!view.getBudgetFilterStartsBeforeDateText().equals("")) {
					whereClause = whereClause + "tbl_budget.startDate <= \""
							+ model.formatDate(view.getBudgetFilterStartsBeforeDateText()) + "\" And ";
				}
				if (!view.getBudgetFilterStartsAfterDateText().equals("")) {
					whereClause = whereClause + "tbl_budget.startDate >= \""
							+ model.formatDate(view.getBudgetFilterStartsAfterDateText()) + "\" And ";
				}
				if (!view.getBudgetFilterEndsBeforeDateText().equals("")) {
					whereClause = whereClause + "tbl_budget.endDate <= \""
							+ model.formatDate(view.getBudgetFilterEndsBeforeDateText()) + "\" And ";
				}
				if (!view.getBudgetFilterEndsAfterDateText().equals("")) {
					whereClause = whereClause + "tbl_budget.endDate >= \""
							+ model.formatDate(view.getBudgetFilterEndsAfterDateText()) + "\" And ";
				}
	...
	/**
	 * It displays a filtered list of the Accounts that are in the database based of
	 * the inputed filters.
	 * 
	 * it will first validate the input, no input is accepted. if there is an issue
	 * will display the issue in an alert dialog box. if there is no issue, it will
	 * display in the accounts list page a list of all the accounts that passed all
	 * the filters. the inputs are from the menu filter page.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * It displays a filtered list of the storage locationss that are in the
	 * database based of the inputed filters.
	 * 
	 * it will first validate the input, no input is accepted. if there is an issue
	 * will display the issue in an alert dialog box. if there is no issue, it will
	 * display in the storage location list page a list of all the storage locations
	 * that passed all the filters. the inputs are from the storage lcoation filter
	 * page.
	 * 
	 * @author Student
	 *
	...
	/**
	 * resets the stock details page it clears all the text feilds gives the combo
	 * box all the storage locations the database has
	 */
	...
	/**
	 * loads the budget list page. loads the budget list page and populate it list
	 * view with all the budgets the database has in string format
	 */
	...
	/**
	 * saves / updates the inputed budget details. validates all the user input. if
	 * there is an issue it make an alert dialog box of the user issues if there
	 * isnt an issues, it see when the page loaded was a selected budget populated,
	 * if it is null it know came from add so just inserts the info, if there is one
	 * it updates that selected budget. it then loads the budget list page.
	 * 
	 * @author Student
	 *
	 */
	...
	private class EHBudgetDetailsBtnSave implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			if (masterError.equals("")) {
				...
				if (view.getBudgetDetailsInputtedStartDateAsLocalDate().getYear() > view
						.getBudgetDetailsInputtedEndDateAsLocalDate().getYear()) {
					isdbed = true;

				} else if (view.getBudgetDetailsInputtedStartDateAsLocalDate().getMonthValue() > view
						.getBudgetDetailsInputtedEndDateAsLocalDate().getMonthValue()) {
					isdbed = true;

				} else if (view.getBudgetDetailsInputtedStartDateAsLocalDate().getDayOfMonth() > view
						.getBudgetDetailsInputtedEndDateAsLocalDate().getDayOfMonth()) {
					isdbed = true;
				...
				if (!isdbed || output.isPresent() && output.get() == ButtonType.OK) {
					...
					if (model.getSelectedBudget() == null) {
						model.addBudget(view.getBudgetDetailsInputtedName(),
								Double.parseDouble(view.getBudgetDetailsInputtedAmount()),
								model.formatDate(view.getBudgetDetailsInputtedStartDate()),
								model.formatDate(view.getBudgetDetailsInputtedEndDate()));
					} else {

						model.updateBudget(view.getBudgetDetailsInputtedName(),
								Double.parseDouble(view.getBudgetDetailsInputtedAmount()),
								model.formatDate(view.getBudgetDetailsInputtedStartDate()),
								model.formatDate(view.getBudgetDetailsInputtedEndDate()));
	...
	/**
	 * saves / updates the account details. validates all the user input. if there
	 * is an issue it make an alert dialog box of the user issues if there isnt an
	 * issues, it see when the page loaded was a selected account populated, if it
	 * is null it know came from add so just inserts the info, if there is one it
	 * updates that account details. it then loads the account list page.
	 * 
		...
		public void handle(ActionEvent event) {
			...
			if (masterError.equals("")) {
				...
				if (model.getSelectedAccount() == null) {
					...
					model.createAccount(view.getAccountDetailsUserName(),
	...
	/**
	 * saves / updates the storage details. validates all the user input. if there
	 * is an issue it make an alert dialog box of the user issues if there isnt an
	 * issues, it see when the page loaded was a selected storage location
	 * populated, if it is null it know came from add so just inserts the info, if
	 * there is one it updates that selected storage location. it then loads the
	 * storage location list page.
	 * 
	 * @author Student
	 *
	 */
	private class EHStorageDetailsBtnSave implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			if (masterError.equals("")) {
				if (model.getSelectedStorageLocation() == null) {
					model.createStorage(view.getStorageDetailsName(), view.getStorageDetailsType(),
							view.getStorageDetailsAvailbilty());
				} else {
					model.updateStorage(view.getStorageDetailsName(), view.getStorageDetailsType(),
							view.getStorageDetailsAvailbilty());
				}
				loadStorgaeLocationListPage();
			} else {
	 ...
	 * check the user input if there is an issue the label is made visible and show
	 * the error if there is no issue the list view is populates with all the
	 * budgets that the database has that is like the user input. like in mysql is %
	 * user input %.
	 * 
	 * @author Student
	 *
	...
	private class EHBudgetListBtnFind implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			 ...
			 * model.getDatabase().getBudgetsThatsLike(view.getBudgetListPage().
			 * getTfFindValue()));
			 * 
	...
	/**
	 * loads the account list page. loads the account list page and populates its
	 * list view with all the accounts the database holds in string format
	 */
	...
	/**
	 * loads the storage location list page. loads the storage location list page
	 * and populates its list view with all the storage location the database has.
	 */
	...
	/**
	 * save stock details that is on a text file. loads a file chooser, if there is
	 * an issue with it eg wrong file type it will show an error message through
	 * alert dialog box, if there is no issue it will save the stock info.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * loads the stock details page. check the user has selected an item from the
	 * list view if no item has been selected the error label is made visible with
	 * info about the error if an item has been selected the stock details page is
	 * loaded and the the pages input areas are populated with the selected stock
	 * info.
	 * 
	 * @author Student
	 *
	...
	/**
	 * adds a ingredient to the dish details list view. it first validates the user
	 * input. if there is an issue it shown in an alert dialog box. if there is no
	 * issue, it first sees if the stock type exists if not it makes it and if it
	 * does exist it see if it needs to update its info. it next will then add then
	 * see if it if the dish name has been changed if it has it updates it, if it
	 * hasn't it simply add the stock info to the dish list it then reset the fields
	...
	private class EHDishDetailsBtnAdd implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			if (masterError.equals("")) {
				...
				// used to tell the user, hey your changing, x/y/z
				if (!(model
						.hasTheStockTypeCostChanged(view.getDishDetailsEstimateCost(),
				...
				if (output == null || output.isPresent() && output.get() == ButtonType.OK) {
					...
					if (model.getTestStockName().equals("null")) {
						...
						model.updateStockTypeCost(view.getDishDetailsIngrdeintName(),
								view.getDishDetailsEstimateCost());

						model.updateStockTypeQuanityType(view.getDishDetailsIngrdeintName(), view.getDishDetailsUnit());
					...
					if (model.getSelectedDish() == null) {
						model.createDish(view.getDishDetailsDishName(), view.getDishDetailsIngrdeintName(),
								view.getDishDetailsEstimateCost(), view.getDishDetailsUnit(),
								view.getDishDetailsQuanity());
						...
						model.selectedDishIngrednitnAdd(view.getDishDetailsIngrdeintName(),
								view.getDishDetailsEstimateCost(), view.getDishDetailsUnit(),
								view.getDishDetailsQuanity());
	...
	/**
	 * loads the delete confirmation page with the selected info. if user hasn't
	 * made a selection it shows an error message
	 * 
	 * @author Student
	 *
	 */
	private class EHDishDetailsBtnDelete implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			if (masterIssue.equals("")) {
				...
				// Alert dishDetailsErrorMessage = model.makeAlert("selection issue",
				// masterIssue);
	...
	/**
	 * loads the selected stock info into the dish details input fields. checks the
	 * user has selected stock from the list view. if no stock has been selected an
	 * alert dialog box is shown with an error message. if an item has been
	 * selected, the item is removed from the listview and its info is populated in
	 * to the user input areas.
	 * 
	 * @author Student
	 *
	 */
		...
		public void handle(ActionEvent event) {
			...
			if (masterIssue.equals("")) {
				...
				view.setDishDetailsUserInput(view.getDishDetailsSelectedItem().getName(),
						view.getDishDetailsSelectedItem().getQuanity(),
						view.getDishDetailsSelectedItem().getQuanityType(),
						view.getDishDetailsSelectedItem().getCost());
	 ...
	 * check the user input if there is an issue a alert dialog box with the error
	 * message is shown. if there is no issue the list view is populates with all
	 * the dishes that the database has that is like the user input. like in mysql
	 * is % user input %.
	 * 
	 * @author Student
	 *
	 ...
	 * check the user input if there is an issue the label is made visible and show
	 * the error if there is no issue the list view is populates with all the menus
	 * that the database has that is like the user input. like in mysql is % user
	 * input %.
	 * 
	 * @author Student
	 *
	...
	/**
	 * loads the menu details page. check the user has selected an item from the
	 * list view if no item has been selected the error label is made visible with
	 * info about the error if an item has been selected the menu details page is
	 * loaded and the the pages input areas are populated with the selected menu
	 * info.
	 * 
	 * @author Student
	 *
	...
	/**
	 * sets the menu name and budget used for the menu. check the user input. if
	 * there is an issue it show an alert dialog box with the error message. if
	 * there is no issue it loads the menu details page.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * adds a dish from the dish list to the menu list and potentially the shopping
	 * list. check the user has selected a budget and given the menu a name, checks
	 * that the user has selected an item from the dish list and the dish total cost
	 * doesnt go over the budget. if it fails any of the before mentioned things, it
	 * shows an alert dialog box with the error message if it passes it will add the
	 * dish to the menu list, it will then depending on if the item stock is in add
	 * the item that stock needs to be order to the shopping list. and finaly remove
	 * the dish from the dish list and decreasing the budget
	 * 
	 * 
	 * @author Student
	 *
	...
	/**
	 * removed a dish from the menu list. check the user has seletced a dish in the
	 * menu list. if no dish has been selected an alert dialog box is shown with the
	 * error message inside. it then removed the dish from the menu list and add it
	 * to the dish list, while also removeing any of its ingredients from the
	 * shopping list and increaing the budget but the dish cost.
	 * 
	 * @author Student
	 *
	 */
	...
	private class EHMenuDetailsBtnRemoveFromList implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			if (masterErrorMessage.equals("")) {
				 ...
				 * model.removeADishFromSelectedMenuDishes(view.
				 * getMenuDetailsMenuListSelectedIndex()); model.resetMenuDetailList();
				 * view.setMenuDetailsMenuListItems(model.getSelectedMenuDishes());
				 * view.setMenuDetailsDishList(model.getNotSelectedDishesAsString());
				 * view.setMenuDetailsShoppingList(model.getSelectedMenuStockType());
				 * view.setMenuDetailsBudgetValue(model.getBudgetSizeMinusTheShoppingList()+"");
	...
	/**
	 * save the dish made in the dish details. check that the dish has item in it.
	 * if no items an alert dialog box is shown with the error message in it. it
	 * then add the dish to the database or update dish depending on where the user
	 * came from. it adds the dish or the updates info the list views and the menu
	 * details page is then loaded.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * It displays a filtered list of the dish items that are in the database based
	 * of the inputed filters.
	 * 
	 * it will first validate the input, no input is accepted. if there is an issue
	 * will display the issue in an alert dialog box. if there is no issue, it will
	 * display in the menu details page a list of all the dish that passed all the
	 * filters. the inputs are from the dish filter page.
	 * 
	 * @author Student
	 *
	 */
	private class EHDishFilterBtnApply implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			if (masterError.equals("")) {
				...
				if (!niaInput.equals("null") || !nibInput.equals("null") || !cltInput.equals("null")
	...
	/**
	 * is a functional interface for outputting the menu list. it check if the menu
	 * details shopping list has items if not it show an error message, if it does
	 * it then displace a file chooser to select the folder to save in before making
	 * the file and outputting it. its output is written English.
	 * 
	 * @author Student
	 *
	 */
	private class EHOutputBtnMenuFromList implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			String masterErrorMessage = "";
			/*
			 * if(view.getMenuDetailsMenuListSize() == 0) { issueFrom = "menu dishes list";
	...
	/**
	 * is a functional interface for outputting the shopping list. it check if the
	 * menu details shopping list has items if not it show an error message, if it
	 * does it then displace a file chooser to select the folder to save in before
	 * making the file and outputting it. its output is written English.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * save the menu made in menu details. check there is a menu info to be saved,
	 * if there isnt it show an error message if there is it checks to see if it
	 * should update the database or make a new entry and use the corresponding menu
	 * functions.
	 * 
	 * @author Student
	 *
	...
	/**
	 * It displays a filtered list of the menu items that are in the database based
	 * of the inputed filters.
	 * 
	 * it will first validate the input, no input is accepted. if there is an issue
	 * will display the issue in an alert dialog box. if there is no issue, it will
	 * display in the menu list page a list of all the menu that passed all the
	 * filters. the inputs are from the menu filter page.
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * event handler for the admin recommendation save button. it saves the data to
	 * the database then loads the home page
	 * 
	 * @author Student
	 *
	 */
	...
	/**
	 * event handler for the chef recommendation delete button, it loads the delete
	 * confirmation page if an item has been selected, else it shows an error
	 * message.
	 * 
	 * @author Student
	 *
